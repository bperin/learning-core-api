...
func TestEvalItemRepository_Update(t *testing.T) {
	...
	t.Run("successful update", func(t *testing.T) {
		updateReq := &eval_items.UpdateEvalItemRequest{
			Prompt: stringPtr("Updated question?"),
			Hint:   stringPtr("This is a hint"),
		}

		updated, err := repo.Update(ctx, created.ID, updateReq)
		require.NoError(t, err)
		assert.Equal(t, created.ID, updated.ID)
		assert.Equal(t, "Updated question?", updated.Prompt)
		assert.Equal(t, created.Options, updated.Options) // Should remain unchanged
		assert.NotNil(t, updated.Hint)
		assert.Equal(t, "This is a hint", *updated.Hint)
	})
	...
	t.Run("not found error", func(t *testing.T) {
		...
		_, err := repo.Update(ctx, nonExistentID, updateReq)
		assert.Error(t, err)
		assert.True(t, eval_items.IsNotFoundError(err))
	})
}
