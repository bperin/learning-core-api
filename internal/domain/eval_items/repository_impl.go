...
// Update updates an evaluation item
func (r *RepositoryImpl) Update(ctx context.Context, id uuid.UUID, req *UpdateEvalItemRequest) (*EvalItem, error) {
	if err := req.Validate(); err != nil {
		return nil, err
	}
	...
	params := store.UpdateEvalItemParams{
		ID: id,
	}

	// Set fields from request or keep existing values
	if req.Prompt != nil {
	...
	if req.CorrectIdx != nil {
		params.CorrectIdx = *req.CorrectIdx
	} else {
		params.CorrectIdx = existing.CorrectIdx
	}

	params.Hint = toNullString(req.Hint)
	if !params.Hint.Valid {
		params.Hint = toNullString(existing.Hint)
	}

	params.Explanation = toNullString(req.Explanation)
	if !params.Explanation.Valid {
		params.Explanation = toNullString(existing.Explanation)
	}
	...
	if req.Metadata != nil {
		...
		params.Metadata = pqtype.NullRawMessage{RawMessage: jsonData, Valid: true}
	} else {
		// Keep existing metadata
		if existing.Metadata != nil {
			jsonData, err := json.Marshal(existing.Metadata)
			if err != nil {
				return nil, fmt.Errorf("failed to marshal existing metadata: %w", err)
			}
			params.Metadata = pqtype.NullRawMessage{RawMessage: jsonData, Valid: true}
		}
	}

	storeItem, err := r.queries.UpdateEvalItem(ctx, params)
	if err != nil {
		return nil, fmt.Errorf("failed to update eval item: %w", err)
	}

	return toDomainEvalItem(&storeItem), nil
}

// Delete deletes an evaluation item
func (r *RepositoryImpl) Delete(ctx context.Context, id uuid.UUID) error {
	err := r.queries.DeleteEvalItem(ctx, id)
	if err != nil {
		return fmt.Errorf("failed to delete eval item: %w", err)
	}
	return nil
}
