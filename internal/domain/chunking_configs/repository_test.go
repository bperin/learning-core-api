...
func createTestUser(t *testing.T, db *sql.Tx) uuid.UUID {
	...
	_, err := db.ExecContext(ctx,
		...
		userID, "chunking-config@example.com", "password123", false, true, false)
	require.NoError(t, err)

	return userID
}

func TestChunkingConfigRepository_CreateActivate(t *testing.T) {
	db, queries, repo, cleanup := setupTestRepo(t)
	defer cleanup()

	ctx := context.Background()
	userID := createTestUser(t, db)
	initial, err := queries.ListChunkingConfigs(ctx)
	require.NoError(t, err)
	initialCount := len(initial)

	active := true
	first, err := repo.Create(ctx, CreateChunkingConfigRequest{
		ChunkSize:    1000,
		ChunkOverlap: 100,
		IsActive:     &active,
		CreatedBy:    userID,
	...
	second, err := repo.Create(ctx, CreateChunkingConfigRequest{
		ChunkSize:    800,
		ChunkOverlap: 80,
		IsActive:     &active,
		CreatedBy:    userID,
	...
	activeCfg, err := repo.GetActive(ctx)
	require.NoError(t, err)
	assert.Equal(t, second.ID, activeCfg.ID)
	...
	activeCfg, err = repo.GetActive(ctx)
	require.NoError(t, err)
	assert.Equal(t, first.ID, activeCfg.ID)
